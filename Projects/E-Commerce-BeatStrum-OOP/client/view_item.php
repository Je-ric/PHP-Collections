...
class ShoppingCart {
    ...
    public function addToCart($itemId, $quantity, $clientId) {
        ...
        if ($check_result->num_rows > 0) {
            ...
            if ($this->conn->query($update_query) === TRUE) {
                ...
                echo "<script>alert('Error updating item quantity in the cart: " . $this->conn->error . "')</script>";
            }
        } else {
            ...
            if ($this->conn->query($insert_query) === TRUE) {
                ...
                echo "<script>alert('Error adding item to the cart: " . $this->conn->error . "')</script>";
            }
        }
    }
}
...
if (isset($_POST['add_to_cart'])) {
    ...
    $shoppingCart->addToCart($itemId, $quantity, $_SESSION['client_id']);
}
?>
...
<head>
    ...
    <style>
    <?php include('C:\xampp\htdocs\PHP-Projects\E-Commerce-BeatStrum-OOP\css\view_item.css'); ?>
    </style>
</head>
<body>    
<?php 
...
<script src="/PHP-Projects/E-Commerce-BeatStrum-OOP/script/quantityButton.js"></script>
</body>
</html>