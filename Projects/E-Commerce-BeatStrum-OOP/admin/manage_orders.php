...
class OrderHandler {
    ...
    public function approveOrder($order_id) {
        ...
        if ($this->conn->query($sql_approve_order)) {
            header("Location: manage_orders.php");
            exit();
        } else {
            echo "Error approving order: " . $this->conn->error;
        }
    }
    ...
    public function getPendingOrders() {
        ...
        return $this->conn->query($sql_pending_orders);
    }
    ...
    public function getApprovedOrders() {
        $sql_approved_orders = "SELECT orders.*, client_accounts.name AS buyer_name, client_accounts.address AS buyer_address, 
                        ...
                        GROUP BY orders.id
                        ORDER BY orders.order_date DESC"; 
        return $this->conn->query($sql_approved_orders);
    }
}
...
if (isset($_GET['order_id'])) {
    ...
    $orderHandler->approveOrder($order_id);
}

$result_pending_orders = $orderHandler->getPendingOrders();
$result_approved_orders = $orderHandler->getApprovedOrders();
...
<head>
    ...
    <style>
        <?php 
        include('C:\xampp\htdocs\PHP-Projects\E-Commerce-BeatStrum-OOP\css\manage_orders.css');
        ?>
</style>
